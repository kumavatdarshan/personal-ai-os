...
export default function Dashboard() {
  ...
  const generatePlan = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/ai/daily-plan');
  ...
  const fetchQuote = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/ai/quote');
  ...
  const fetchWeeklySummary = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/ai/weekly-summary');
  ...
  const sendChat = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/ai/chat', { message: chatMsg });
          ...
          <div style={{ marginTop: '48px', paddingTop: '32px', borderTop: '1px solid #111' }}>
            ...
            <div style={{ display: 'flex', gap: '0', marginBottom: '24px', border: '1px solid #222' }}>
              ...
              <button onClick={addTask} style={{
                ...
                color: '#888', fontSize: '10px', letterSpacing: '2px',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                {tasks.items.map(task => (
                  <div key={task.id} style={{
                    ...
                    >
                      {task.done && (
                        <div style={{ color: '#000', fontSize: '12px', fontWeight: '700', lineHeight: 1 }}>✓</div>
                      )}
                    </div>

                    {/* Task text */}
                    <div style={{
                      flex: 1, fontSize: '14px', color: task.done ? '#444' : '#fff',
            ...
            {tasks.items.some(t => t.done) && (
              <button
                onClick={() => setTasks(prev => ({ ...prev, items: prev.items.filter(t => !t.done) }))}
                style={{
            ...
            {showChat && (
              <div>
                <div style={{ display: 'flex', border: '1px solid #222' }}>
                  <input
                    type="text"
                    placeholder="Ask anything about your goals, habits, day..."
                    value={chatMsg}
                ...
                {chatReply && (
                  <div style={{
                    marginTop: '16px', paddingLeft: '16px', borderLeft: '1px solid #222',
                    fontSize: '14px', color: '#fff', lineHeight: '1.8',
                  }}>
                    {chatReply}
                  </div>
                )}
        ...
        <div>
              ...
              goals.map(goal => (
                <div key={goal._id} style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <span style={{ fontSize: '12px', color: '#fff', letterSpacing: '0.5px' }}>{goal.title}</span>
          ...
          <div style={{ padding: '32px 24px' }}>
            ...
            <div style={{ fontSize: '56px', fontWeight: '800', letterSpacing: '-3px', lineHeight: '1', color: '#fff' }}>
              {avgProgress}
              <span style={{ fontSize: '20px', color: '#555', fontWeight: '300', letterSpacing: '0' }}>%</span>
            </div>
