...
export default function Goals() {
  ...
  const addGoal = async () => {
    ...
    try {
      await axios.post('http://localhost:5000/api/goals', { title, description, deadline });
  ...
  const updateProgress = async (id, progress) => {
    try {
      await axios.patch(`http://localhost:5000/api/goals/${id}/progress`, { progress: Number(progress) });
      fetchGoals();
    } catch (err) {
      console.error(err);
    }
